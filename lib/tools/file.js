...
exports.readDir = function (dirpath) {
  var suppuort_format = ['.md', '.mkd', '.markdown'];
  dirpath = path.resolve(process.cwd(), dirpath);
  try {
    return fs.readdirSync(dirpath).filter(function (eachfile) {
      return fs.statSync(exports.absPath(eachfile)).isFile() && _.contains(suppuort_format, path.extname(eachfile));
    });
  } catch (e) {
...
exports.readFile = function (filepath) {
  try {
    return fs.readFileSync(exports.absPath(filepath)).toString();
  } catch (e) {
    log.error(e);
    return null;
  }
